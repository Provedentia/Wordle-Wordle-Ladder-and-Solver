def find_dislikes(friends: dict)->set[tuple[str]]:
    ...
    for key in friends:
        for i in range(len(key)):
            temp_tup = sorted(key, key[i])
            dislike_set.add(temp_tup)
    return dislike_set
    ### END SOLUTION
...
def filter_no_dislikes(friends:dict)->tuple[list[str], dict]:
    ...
    ### BEGIN SOLUTION
    nice_list =  []
    friends_4 = friends.copy()
    for key in friends_4.keys():
        if len(friends_4[key]) == 0:
            nice_list.append(key)
    for i in range(len(nice_list)):
        friends_4.pop(nice_list[i])
    return nice_list, friends
    ### END SOLUTION
...
def filter_bad_invites(all_subsets:list, friends:dict)->list[list[str]]:
    ...
    for subset in all_subsets:
        ...
        for friend in friends:
            for values in friends[friend]:
                if friend in subset and values in subset:
                    valid = False
                    break
        if valid:
            valid_invite.append(subset)
    return valid_invite
...
def invite_to_dinner_optimized(friends:dict)->list[str]:
    ...
    x = ['']
    friends_4 = friends.copy()
    friend_check = friends.copy()
    nice_list, friends_4 = filter_no_dislikes(friends_4)
    while len(x) > 0:
        x = max(k for k in friends_4.values())
        if len(x) == 0:
            break
        else:
            y = ""
            for k in friends_4.keys():
                if friends_4[k] == x:
                    y = k
            friends_4.pop(y)
        for key in friends_4:
            if y in friends_4[key]:
                friends_4[key].remove(y)
        friends_4 = filter_no_dislikes(friends_4)[1]
        
    best_list = []
    for key in friend_check.keys():
        if key not in friends_4.keys():
