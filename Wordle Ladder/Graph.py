...
class Graph:
    def __init__(self, adjacency_list:dict[str, set[str]]):
        ...
        for i in adjacency_list.keys():
           for x in adjacency_list[i]:
              newjawnt = [i, x]
              newjawnt = sorted(newjawnt)
              jawntlist.add(tuple(newjawnt))
        self.__edges = jawntlist
        ### END SOLUTION
    ...
    def get_all_shortest_bfs_paths(self, source:str, target:str) -> set[tuple[str]]:
        ...
        shortest_paths = set()
        shortest_length = self.get_shortest_bfs_path_length(source, target)

        if shortest_length == 0 or not self.is_vertex(source) or not self.is_vertex(target):
        ...
        while path_queue:
            ...
            for neighbour in self.get_neighbors(last_node):
                ...
                if neighbour == target and len(new_path) - 1 == shortest_length:
                    shortest_paths.add(tuple(new_path))
